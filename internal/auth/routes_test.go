...
const (
	handlerBase = "github.com/PakornBank/go-backend-example/internal/auth.Handler."
)
...
func TestRegisterRoutes(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantRoutes: map[string]routeInfo{
				"/auth/register": {
					handler: handlerPath("Register"),
					method:  "POST",
				},
				"/auth/login": {
					handler: handlerPath("Login"),
					method:  "POST",
				},
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, len(tt.wantRoutes), len(routes))
